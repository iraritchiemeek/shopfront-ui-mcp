...
export function BottomBar({ totalItems, totalPrice, onCheckout }: Props) {
  ...
  return (
    <div className="sticky bottom-0 mt-8 flex items-center justify-between gap-4 rounded-xl bg-white px-6 py-5 shadow-lg dark:bg-slate-800 dark:shadow-none">
      ...
      <button
        type="button"
        onClick={onCheckout}
        className="cursor-pointer rounded-lg bg-brand px-6 py-3 text-base font-bold text-white transition-colors hover:bg-brand-hover"
      >
        Checkout
      </button>
    </div>
